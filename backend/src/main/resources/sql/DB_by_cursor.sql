...
CREATE TABLE t_user (
    ...
    user_type CHAR(1) NOT NULL COMMENT '用户类型：U-普通用户，C-托运公司，A-管理员',
    user_address VARCHAR(63) COMMENT '用户地址，用于托运服务',
    picture_id VARCHAR(18) COMMENT '用户头像ID，关联图片表',
    company_id VARCHAR(18) COMMENT '公司ID，关联公司表，仅托运公司用户有值',
...
CREATE TABLE t_company (
    ...
    company_local VARCHAR(63) COMMENT '公司地址，用于托运服务',
    create_time DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    update_time DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间'
);
...
CREATE TABLE t_order (
    ...
    user_id VARCHAR(18) NOT NULL COMMENT '用户ID，关联用户表，表示订单创建者',
    company_id VARCHAR(18) NOT NULL COMMENT '公司ID，关联公司表，表示承接托运的公司',
    order_status CHAR(1) NOT NULL COMMENT '订单状态：P-待支付，W-待接单，T-运输中，C-已完成，X-已取消',
    pet_status CHAR(1) NOT NULL COMMENT '宠物状态：N-正常，A-异常',
    delivery_status CHAR(1) NOT NULL COMMENT '运输状态：P-待接单，T-运输中，D-已送达',
...
CREATE TABLE t_picture (
    ...
    picture_usage VARCHAR(63) NOT NULL COMMENT '图片用途：A-头像，P-宠物照片，O-订单照片',
    picture_url VARCHAR(255) NOT NULL COMMENT '图片URL，存储图片的访问路径',
...
CREATE TABLE t_comment (
    ...
    user_id VARCHAR(18) NOT NULL COMMENT '用户ID，关联用户表，表示评论者',
...
CREATE TABLE t_system_log (
    ...
    user_id VARCHAR(18) NOT NULL COMMENT '用户ID，关联用户表，表示操作者',
    operation_type VARCHAR(63) NOT NULL COMMENT '操作类型，如登录、修改信息等',
    operation_content TEXT COMMENT '操作内容，详细描述操作信息',
